...
import { getMembersEnrolledThisMonth, getUsers } from "@/app/actions/user";
import { MembersTable } from "@/components/Tables/MembersTable";

export default async function MembersPage() {
  const members = await getUsers(["MEMBER"]);
  const instructors = await getUsers(["INSTRUCTOR"]);
  const newMembersCount = await getMembersEnrolledThisMonth();
      ...
      <div className="container mx-auto flex flex-col gap-8">
        <div className="flex items-center justify-between">
          <div className="space-y-1">
            <p className="text-3xl font-title font-bold">Members</p>
            <p className="text-neutral-600">
              Manage your members, view profiles and track memberships.
            </p>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="bg-black rounded-xl p-4 flex w-full justify-between items-start">
            <div className="space-y-1">
              ...
              <p className="text-xl text-white">{members.length}</p>
